...
class _OutwardEntryScreenState extends State<OutwardEntryScreen> {
  ...
  Future<void> _loadAllInventory() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${l10n?.errorLoadingInventory ?? "Error loading inventory"}: $e')),
      );
    } finally {
  ...
  Future<void> _showOutwardDialog(Map<String, dynamic> item) async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text('${l10n?.outwardEntry ?? "Outward"}: ${item['crop_name'] ?? 'Unknown'}'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('${l10n?.party ?? "Party"}: ${item['person_name'] ?? 'Unknown'}'),
            Text('${l10n?.available ?? "Available"}: ${remainingQty.toStringAsFixed(2)} ${item['packaging_type'] ?? ''}'),
            const SizedBox(height: 16),
            TextField(
              controller: _quantityController,
              keyboardType: TextInputType.number,
              decoration: InputDecoration(
                labelText: l10n?.quantityToRemove ?? 'Quantity to remove',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text(l10n?.confirmOutward ?? 'Confirm Outward'),
          ),
        ],
      ),
    );
  ...
  Future<void> _processOutward(Map<String, dynamic> item) async {
    ...
    if (quantity == null || quantity <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n?.pleaseEnterValidQuantity ?? 'Please enter a valid quantity')),
      );
      return;
    }
    ...
    if (quantity > remainingQty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n?.quantityExceedsStock(remainingQty.toString()) ?? 'Quantity exceeds available stock ($remainingQty)')),
      );
      return;
    }
        ...
        builder: (dialogContext) => AlertDialog(
          title: Row(
            children: [
              ...
              Text(l10n?.outwardRecorded ?? 'Outward Recorded'),
            ],
          ),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
               Text(l10n?.outwardReceiptGenerated(receiptNumber) ?? 'Outward receipt #$receiptNumber generated.'),
                   ...
                   onPressed: () {
                      ...
                      ReceiptService.generateOutwardReceipt(
                          entryData: {
                              ...
                              'quantity': quantity.toString(),
                              'unit': item['packaging_type'] == 'bori' ? 'Bags' : 'MT',
                    ...
                    child: Row(
                      children: [
                        Column(
                          ...
                          children: [
                            Text(
                              l10n?.outwardEntry ?? 'Outward Entry',
                            ...
                            Text(
                              l10n?.stockUnloadingEntry ?? 'Stock unloading entry',
                              style: const TextStyle(
                                fontSize: 14,
                ...
                ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        Text(l10n?.processingOutwardEntry ?? 'Processing outward entry...'),
                      ],
                    ),
                  )
                : ListView(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              l10n?.selectBookingParty ?? '1. Select Booking / Party',
                              style: const TextStyle(
                                fontSize: 18,
                                fontWeight: FontWeight.bold,
                            ...
                            const SizedBox(height: 20),
                            Text(
                              l10n?.searchStoredInventory ?? 'Search Stored Inventory',
                              style: const TextStyle(
                                fontSize: 16,
                            ...
                            TextField(
                              ...
                              decoration: InputDecoration(
                                hintText: l10n?.searchByPartyPhoneCrop ?? 'Search by party, phone, or crop',
                        ...
                        Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 16),
                              Text(
                                l10n?.noInventoryFound ?? 'No inventory found',
                                style: TextStyle(
                                  fontSize: 16,
                          ...
                          children: [
                            Padding(
                              ...
                              child: Row(
                                ...
                                children: [
                                  Text(
                                    l10n?.availableInventory ?? 'Available Inventory',
                                    style: const TextStyle(
                                      fontSize: 16,
                                      fontWeight: FontWeight.w600,
                                      color: Color(0xFF333333),
                                  ...
                                  Text(
                                    l10n?.itemsCount(_searchResults.length) ?? '${_searchResults.length} items',
                                    style: TextStyle(
                                      fontSize: 14,
                                      color: Colors.grey.shade600,
                                    ),
                            ...
                            ..._searchResults.map((result) => _buildInventoryCard(result, l10n)).toList(),
                          ],
                        ),
                    ],
                  ),
  ...
  Widget _buildInventoryCard(Map<String, dynamic> item, AppLocalizations? l10n) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Text(
                          '${l10n?.available ?? "Available"}: ${remainingQty.toStringAsFixed(2)} $packagingType',
                          style: const TextStyle(
                            fontSize: 13,
